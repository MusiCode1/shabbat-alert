<script lang="ts">
  ...
  const bgColors: Record<AlertState, string> = {
    IDLE: "bg-green-600/10 dark:bg-green-500/10 border-green-500/20 shadow-[0_0_30px_rgba(34,197,94,0.1)]",
    ALERT:
      "bg-amber-glow-alert-light dark:bg-amber-glow-alert-dark border-red-500/30 shadow-[0_0_50px_rgba(220,38,38,0.4)]",
    SHELTER:
      "bg-amber-glow-primary-light dark:bg-amber-glow-primary-dark border-orange-500/30 shadow-[0_0_30px_rgba(234,88,12,0.3)]",
    ALL_CLEAR:
      "bg-green-500/20 dark:bg-green-400/20 border-green-400/30 shadow-[0_0_40px_rgba(74,222,128,0.2)]",
  };

  const textColors: Record<AlertState, string> = {
    IDLE: "text-stone-700 dark:text-stone-300",
    ALERT: "text-white",
    SHELTER: "text-white",
    ALL_CLEAR: "text-green-900 dark:text-green-300",
  };
</script>

<div
  class="relative flex min-h-[40vh] flex-col items-center justify-center gap-4 rounded-[2rem] p-8 transition-all duration-700 border backdrop-blur-xl
...
>
  ...
  <div class="mb-2 md:mb-4 flex items-center justify-center text-center">
    ...
    {:else if appState.status === "SHELTER"}
      ...
      />
    {:else if appState.status === "ALL_CLEAR"}
      <Check
        size="clamp(6.5rem, 14vmin, 9rem)"
        strokeWidth={2}
